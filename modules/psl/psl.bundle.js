(() => {
  ...
  // index.js
  var rulesByPunySuffix = rules_default.reduce(
    (map, rule) => {
      const suffix = rule.replace(/^(\*\.|\!)/, "");
      const punySuffix = import_punycode.default.toASCII(suffix);
  ...
  var findRule = (domain) => {
    const punyDomain = import_punycode.default.toASCII(domain);
  ...
  var validate = (input) => {
    const ascii = import_punycode.default.toASCII(input);
    if (ascii.length < 1) {
  ...
  var parse = (input) => {
    ...
    const handlePunycode = () => {
      ...
      if (parsed.domain) {
        parsed.domain = import_punycode.default.toASCII(parsed.domain);
      }
      if (parsed.subdomain) {
        parsed.subdomain = import_punycode.default.toASCII(parsed.subdomain);
      }
      return parsed;
    };
  ...
  var index_default = { parse, get, isValid };
})();
