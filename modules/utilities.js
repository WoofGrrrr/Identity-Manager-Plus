...
//var M  = ua.match(/(firefox|thunderbird(?=\/))\/?\s*([^\s]*)\s*/i) || [];
  var M  = ua.match(/(thunderbird(?=\/))\/?\s*([^\s]*)\s*/i) || []; // cannot add "firefox|" in there because Thunderbird has BOTH

  if (M.length > 2) {
    var version = M[2].split('\.');
    if (version.length > 0) M = M.concat(version);
  }
...
// requires permissions!!!  MABXXX Should this be part of the IcIdentities class???
// - accountsIdentities
export async function getIdentityEmail(identityId) {
  let email = null;
...
// requires permissions!!!  MABXXX Should this be part of the IcIdentities class???
// - accountsIdentities
export async function getIdentityName(identityId) {
  let name = null;
...
// requires permissions!!!  MABXXX Should this be part of the IcIdentities class???
// - accountsIdentities
export async function getIdentityNameAndEmail(identityId) {
  let nameAndEmail = null;

  if (identityId) {
    let identity = await messenger.identities.get(identityId);
    if (identity) {
      if (identity.name) {
        nameAndEmail = identity.name + " <" + identity.email + ">";
      } else {
        nameAndEmail = "<" + identity.email + ">";
      }
    }
  }
...
// - accountsIdentities
// - compose
export async function getDefaultComposeToNameAndEmail(tabId) {
  if (tabId) {
    let composeDetails = await messenger.compose.getComposeDetails(tabId);

    if (composeDetails) {
      return await getIdentityNameAndEmail(composeDetails.identityId);
    }
  }
